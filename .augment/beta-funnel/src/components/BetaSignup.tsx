...
import { MapPin, Users, Car, Clock, KeyRound, Mail, ArrowRight, Check, Shield, ExternalLink } from 'lucide-react';
...
function HeaderSection({ standalone }: { standalone: boolean }) {
  return (
    <div className="text-center space-y-4">
      ...
      <p className="text-[11px] font-medium text-white/30 uppercase tracking-widest">Powered by live venue and parking data</p>
    </div>
  );
}
      ...
      <div className="relative rounded-2xl overflow-hidden backdrop-blur-xl border border-white/15 bg-[#1C1C1E]/60 shadow-2xl">
        <div className="h-24 bg-gradient-to-br from-purple-900/40 to-black/40 flex items-end p-4 relative">
          ...
          <div className="relative z-10 w-full flex justify-between items-end">
            <div><h3 className="text-white font-bold text-lg">Midtown Venue</h3><div className="flex items-center gap-1 text-white/60 text-xs"><MapPin className="w-3 h-3" /><span>Atlanta, GA</span></div></div>
    ...
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isSubmitting} className="w-full relative overflow-hidden rounded-xl p-4 bg-gradient-to-r from-purple-600 to-cyan-600 text-white font-bold text-lg shadow-lg shadow-purple-500/25 disabled:opacity-70 disabled:cursor-not-allowed group">
        <div className="relative z-10 flex items-center justify-center gap-2">
          {isSubmitting ? (<div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />) : (<><span>Get Early Access</span><ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" /></>)}
        </div>
