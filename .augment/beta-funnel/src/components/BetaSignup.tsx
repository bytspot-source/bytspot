  ...
  return (<>
    ...
    <div className={`min-h-screen relative overflow-hidden flex flex-col items-center justify-center px-6 py-12 ${isDarkMode ? 'bg-[#000000]' : 'bg-[#F5F7FA]'}`}>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-[-10%] left-[-10%] w-[280px] h-[280px] sm:w-[400px] sm:h-[400px] md:w-[500px] md:h-[500px] rounded-full blur-[80px] sm:blur-[100px] opacity-30 ${isDarkMode ? 'bg-purple-600' : 'bg-purple-400'}`} />
        <div className={`absolute bottom-[-10%] right-[-10%] w-[280px] h-[280px] sm:w-[400px] sm:h-[400px] md:w-[500px] md:h-[500px] rounded-full blur-[80px] sm:blur-[100px] opacity-20 ${isDarkMode ? 'bg-cyan-600' : 'bg-cyan-400'}`} />
        {standalone && <div className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] sm:w-[450px] sm:h-[450px] md:w-[600px] md:h-[600px] rounded-full blur-[100px] sm:blur-[120px] opacity-10 bg-purple-500" />}
      </div>
...
function HeaderSection() {
  return (
    <div className="text-center space-y-4">
      ...
      <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white tracking-tight leading-[1.1]">Know{' '}<span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-cyan-400">Before</span>{' '}You Go.</h1>
      <p className="text-[15px] sm:text-[17px] text-white/70 max-w-sm mx-auto leading-relaxed">Bytspot shows live crowd levels, parking availability, and ride options — all in one place.</p>
      {/* P2: Feature bullets */}
...
function ProductMockCard() {
  return (
    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="relative mx-1 sm:mx-4">
