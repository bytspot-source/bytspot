...
import { VenueInsider } from "./components/VenueInsider";
...
function App() {
  ...
  const [currentState, setCurrentState] =
    useState<AppState>("splash");
  const [userData, setUserData] = useState<UserData>({});
  const [pendingConcierge, setPendingConcierge] = useState(false);
  const [pendingConciergeQuery, setPendingConciergeQuery] = useState<string | null>(null);
  ...
  const [hasSeenSplash, setHasSeenSplash] = useState<boolean>(() =>
    typeof window !== 'undefined' && localStorage.getItem('bytspot_seen_splash') === 'true'
  );

  // Splash advance is controlled by SplashScreen via onDone (approx 4.5s)
  // Removed local timer to avoid early transition.
  ...
  const showFooter = ["discovery", "swipe", "map", "concierge", "profile", "insider"].includes(currentState);
  const connectivity = { isOnline: typeof navigator !== 'undefined' ? navigator.onLine : true, isSlowConnection: false };
  const trackEvent = (event: string, properties?: Record<string, any>) => {
    try { console.debug('[trackEvent]', event, properties || {}); } catch {}
  };
  ...
  return (
    <AppErrorBoundary onError={handleError}>
      ...
      >
        ...
        {/* Main Content */}
            {currentState === "splash" && (
              <SplashScreen totalMs={hasSeenSplash ? 4500 : 6000} onDone={() => { localStorage.setItem('bytspot_seen_splash','true'); setHasSeenSplash(true); setCurrentState(isAuthenticated ? "home" : "auth"); }} />
            )}
        ...
        <main id="main-content" className="relative z-10">
          <AnimatePresence mode="wait">
            {currentState === "home" && (
              <LandingPage
                key="home"
                onGetStarted={() => handleStateChange(isAuthenticated ? "discovery" : "auth")}
                onBecomeHost={handleHostOnboarding}
                onSignIn={() => handleStateChange("auth")}
                onAskConcierge={(query) => {
                  setPendingConcierge(true);
                  setPendingConciergeQuery(query || null);
                  handleStateChange(isAuthenticated ? "concierge" : "auth");
                }}
              />
            )}
            ...
            {currentState === "registration" && (
              <RegistrationFlow
                ...
                onComplete={() => {
                  const next = pendingConcierge ? "concierge" : "discovery";
            ...
            {currentState === "discovery" && (<div data-testid="discover-page">
              <DiscoverySearch
                key="discovery"
                userData={userData}
                onComplete={() => {
            ...
            {currentState === "concierge" && (<div data-testid="concierge-page">
              <ConciergeChat
                key="concierge"
            ...
            {currentState === "map" && (<div data-testid="map-page">
              <MapInterface
                key="map"
                userData={userData}
                onBack={() => handleStateChange("swipe")}
            ...
            {currentState === "insider" && (
              <div data-testid="insider-page">
                <VenueInsider
                  onBack={() => handleStateChange("swipe")}
                  onConcierge={() => handleStateChange("concierge")}
                />
              </div>
            )}

            {currentState === "profile" && (<div data-testid="profile-page">
        ...
        <EnhancedFooter
          ...
          onOpenInsider={() => handleStateChange("insider")}
          onOpenConcierge={() => handleStateChange("concierge")}
          onOpenProfile={() => handleStateChange("profile")}
          userData={userData}
        ...
        <MapMenuSlideUp
          ...
          onSelect={(action) => {
            ...
            if (action === 'smart_parking' || action === 'ai_navigation' || action === 'live_venue_data') {
              handleStateChange('map');
            }
