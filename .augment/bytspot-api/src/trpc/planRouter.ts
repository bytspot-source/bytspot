...
import { candidatesFromPlan, candidatesFromDiscovery, discoverablePartyWhere, filterDiscoverableParties, partyTablePriceFloors, type PartyFacts, type PlanItemFacts, type DiscoverablePartyFacts } from '../services/primePathCandidates';
  ...
  bookables: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if ((BOOKABLE_CATEGORIES as readonly string[]).includes(input.category)) {
        ...
        const tableFloors = await partyTablePriceFloors(shown.map((party) => party.id), new Date());
        return { offerings: shown
          .map((party) => {
            ...
            return {
              ...
              // Null when this Party sells no table a guest could still take,
              // which is a different fact from a table that costs nothing.
